"""GradMax on Chesseract without TFDS registration.

This wrapper reuses the CIFAR training pipeline
`tfds.builder(...)` so that `config.dataset=chesseract` loads local NPY files
...
from growneuron.chesseract import data as chesseract_data
from growneuron.chesseract import wide_resnet as chesseract_wide_resnet


class _ChesseractBuilder:
    ...
    def __init__(self, root_dir: str):
        ...
        # Input shape after NHWC conversion: (8, 8, 12)
    ...
    def as_dataset(
        ...
        if split == 'train':
            ...
            raise ValueError(
                f'Unsupported split for Chesseract builder: {split}')
...
def _resolve_chesseract_root(data_dir: Optional[str]) -> str:
    """Resolve dataset root that contains Chesseract_extracted/*.npy files."""
    candidates = []
    if data_dir:
        candidates.extend([
            os.path.join(data_dir, 'Chesseract_extracted'),
            os.path.join(data_dir, 'datasets', 'Chesseract_extracted'),
        ])
    candidates.extend([
        os.path.join(
            os.path.expanduser('~'), 'data', 'datasets', 'Chesseract_extracted'),
        '/scratch/sdouka/data/Chesseract_extracted',
    ])
    ...
    raise FileNotFoundError(
        'Could not find Chesseract files. Checked: ' + ', '.join(candidates))
...
def _patched_builder(name: str, *args, **kwargs):
    if name == 'chesseract':
        root = _resolve_chesseract_root(cifar_main.FLAGS.data_dir)
        return _ChesseractBuilder(root)
    return _ORIGINAL_TFDS_BUILDER(name, *args, **kwargs)


tfds.builder = _patched_builder
# Use dataset-specific WRN head geometry.
cifar_main.data = chesseract_data
cifar_main.wide_resnet = chesseract_wide_resnet
