"""GradMax on MultiNIST without TFDS registration.

This wrapper reuses the CIFAR training pipeline
`tfds.builder(...)` so that `config.dataset=multnist` loads local NPY files
...
from growneuron.multnist import data as multnist_data
from growneuron.multnist import wide_resnet as multnist_wide_resnet


class _MultNISTBuilder:
    ...
    def __init__(self, root_dir: str):
        ...
        # Data files are stored as (N, C, H, W). Convert to (N, H, W, C) for TF.
        ...
        # Input shape after NHWC conversion: (28, 28, 3)
    ...
    def as_dataset(
        ...
        if split == 'train':
            ...
            raise ValueError(
                f'Unsupported split for MultiNIST builder: {split}')
...
def _resolve_multnist_root(data_dir: Optional[str]) -> str:
    """Resolve dataset root that contains MultiNIST_extracted/*.npy files."""
    candidates = []
    if data_dir:
        candidates.extend([
            os.path.join(data_dir, 'MultNIST_extracted'),
            os.path.join(data_dir, 'datasets', 'MultNIST_extracted'),
        ])
    candidates.extend([
        os.path.join(
            os.path.expanduser('~'), 'data', 'datasets', 'MultNIST_extracted'),
        '/scratch/sdouka/data/MultNIST_extracted',
    ])
    ...
    raise FileNotFoundError(
        'Could not find MultiNIST files. Checked: ' + ', '.join(candidates))
...
def _patched_builder(name: str, *args, **kwargs):
    if name == 'multnist':
        root = _resolve_multnist_root(cifar_main.FLAGS.data_dir)
        return _MultNISTBuilder(root)
    return _ORIGINAL_TFDS_BUILDER(name, *args, **kwargs)


tfds.builder = _patched_builder
# Use MultiNIST-specific input pipeline and WRN head geometry.
cifar_main.data = multnist_data
cifar_main.wide_resnet = multnist_wide_resnet
